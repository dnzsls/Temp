# =============================================================================
# HAFTALIK MIP PİPELİNE - ACTUAL V7 (WEEKLY, HAFTAİÇİ)
# =============================================================================
#
# V7 — STANDALONE pipeline. v6'nın tüm fonksiyonları bu dosyada (load_aht,
# prepare_calls_30, calculate_erlang_all, optimize_queue, print_queue_report,
# distribute_surplus, vs.) → run_weekly_v7.py sadece bu dosyadan import eder.
#
# V7'nin v6 üzerine getirdiği YENİ özellik (dosyanın altında):
#
#   1) HAFTALIK MIP (optimize_week + run_week_all_queues):
#      - Pzt-Cum tek MIP, stabil vardiya staffing'i (agent atama doğrudan).
#      - df_shifts'e opsiyonel 'available_days' kolonu desteklenir
#        (örn. ['Fri'] = sadece Cuma aktif).
#      - Stable vardiyalar tüm günlerde aynı kişi sayısı; gün-özel vardiyalar
#        sadece kendi günlerinde açılır.
#      - Her gün için v6-uyumlu mip_info üretir → print_queue_report ile
#        günlük detay raporu basılır.
#
#   V6'dan miras alınan tüm günlük özellikler (rr_penalty, slot_cap,
#   balance_penalty, pencere bazlı start_smoothing, hourly_report, vs.)
#   weekly MIP'te de geçerli, her gün için ayrı uygulanır (smoothing/balance
#   ilk iterasyonda günlük uygulanmıyor, sonraki sürümde eklenecek).
#
# =============================================================================
...
def optimize_week(erlang_by_slot_per_day, df_shifts, queue, target_dates, config,
                  ...
                  min_per_shift_override=None, verbose=True):
    ...
    M = 500
    min_default = (min_per_shift_override
                   if min_per_shift_override is not None
                   else mcfg['min_per_shift'])
...
def run_week_all_queues(df_calls, df_shifts_by_queue, target_dates, config,
    ...
    for queue in queues:
        ...
        # Shrinkage fallback yapılandırması
        ...
        # Denenecek decrement listesi: 0.0 (orijinal), step, 2×step, ...,
        # son adım tüm değerleri floor'a indirecek kadar büyük.
        trial_decrements = [0.0]
        if wsf_enabled and max_shr > floor:
            d_val = step
            while d_val < max_shr:
                trial_decrements.append(round(d_val, 4))
                d_val += step
            trial_decrements.append(round(max(d_val, max_shr), 4))

        stable = day_specific = info_per_day = None
        solution_stage = None
        used_decrement = 0.0
        trial_log = []  # raporda göstermek için (stage, decrement, erl_total, status)

        for idx, decrement in enumerate(trial_decrements):
            if decrement == 0.0:
                cfg_used = config
                ...
                cfg_used = copy.deepcopy(config)
                cfg_used['queue_configs'][queue]['erlang']['shrinkage'] = new_shrinkage
                stage_label = f'shrinkage_decrement={decrement:.2f}'

            erlang_by_slot_per_day = _build_erlang_per_day(
                df_calls_30, queue, target_dates, cfg_used)
            erl_total = sum(sum(v.values()) for v in erlang_by_slot_per_day.values())

            if verbose:
                if decrement == 0.0:
                    print(f"  [shrinkage trial #{idx}] orijinal shrinkage "
                          f"(max={max_shr:.2f}) → Erlang toplam={erl_total}")
                else:
                    print(f"  [shrinkage trial #{idx}] decrement -{decrement:.2f} "
                          f"(floor={floor}) → Erlang toplam={erl_total}")

            stable_try, day_try, info_try = optimize_week(
                erlang_by_slot_per_day, df_shifts, queue, target_dates,
                cfg_used, verbose=verbose,
            )

            status = 'OPTIMAL' if stable_try is not None else 'INFEASIBLE'
            trial_log.append({
                'idx': idx, 'stage': stage_label, 'decrement': decrement,
                'erlang_total': erl_total, 'status': status,
            })

            if stable_try is not None:
                stable, day_specific, info_per_day = stable_try, day_try, info_try
                used_decrement = decrement
                solution_stage = stage_label
                if verbose:
                    if decrement == 0.0:
                        print(f"  ✓ Orijinal shrinkage ile çözüldü")
                    else:
                        print(f"  ✓ Çözüm bulundu: shrinkage -{decrement:.2f}")
                break

            if not wsf_enabled:
                # Fallback kapalıysa orijinal denemeden sonra durulur
                break
            ...
            if verbose:
                ...
                if wsf_enabled:
                    print(f"  ✗ {queue}: MIP infeasible — shrinkage fallback de "
                          f"çözmedi (min_per_shift={mn}, denenen kademe sayısı="
                          f"{len(trial_log)})")
                else:
                    print(f"  ✗ {queue}: MIP infeasible (min_per_shift={mn} ile "
                          f"çözülemedi, weekly_shrinkage_fallback kapalı)")

            # Rapor: hangi kademede ne oldu (kullanıcı görmek istedi)
            if verbose and trial_log:
                print(f"\n  ┌─ Shrinkage Fallback Denemeleri ({queue}) ─┐")
                print(f"  │ {'#':>2} {'Kademe':<32} {'Erlang':>8} {'Sonuç':<11} │")
                for t in trial_log:
                    print(f"  │ {t['idx']:>2} {t['stage']:<32} "
                          f"{t['erlang_total']:>8} {t['status']:<11} │")
                print(f"  └{'─' * 60}┘")
            continue
        ...
        if verbose:
            ...
            print(f"  │ {'#':>2} {'Kademe':<32} {'Erlang':>8} {'Sonuç':<11} │")
            for t in trial_log:
                marker = ' ←' if t['decrement'] == used_decrement else '  '
                print(f"  │ {t['idx']:>2} {t['stage']:<32} "
