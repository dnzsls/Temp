...
from actual_pipeline_v7_weekly import (
    ...
    print_weekly_full_report,   # ZENGİN rapor (günlük slot detay + RR%)
    ...
    'queue_configs': {
        ...
        'kitle': {
            ...
            'mip': {
                ...
                # Fallback kapalı — 13'ün altına inilmez
                'min_per_shift_fallback': {'enabled': False},
...
# %% [HÜCRE 9] — ZENGİN haftalık rapor
# Her queue için:
#   - Stable vardiya listesi
#   - Gün-özel ekler
#   - Günlük özet metrikler (Çağrı, Erlang, MIP, In, Out, NMT, Ç.Kap, RR%)
#   - Her gün için slot bazlı detay tablosu
#   - Penalty özetleri (RR_excess, SC_excess)
print_weekly_full_report(
    results=results,
    df_calls=df_calls,
    config=CONFIG_WEEKDAY,
    queues=('kitle', 'kurumsal', 'gold'),
