...
def _resolve_kadro(kadro_value, date_str=None, day_label=None):
    """V9 — Kadro tavanını 3 katmanlı çözer:
        tarih > hafta_N > default

    Not (V9 değişikliği): Mon..Sun (gün-of-week) layer'ı KALDIRILDI.
    Sebep: weekly MIP stable inhouse kuralı gereği Pzt-Cum aynı kişi sayısı
    kullanır. Aynı hafta içinde günden güne farklı kadro vermek stable
    inhouse'u en kısıtlı güne zorlar; etki çok sınırlıydı, kafa karıştırıcıydı.
    ...
    kadro_value: int (tüm günlerde aynı) VEYA dict.
        Dict ise key'ler:
          ...
          - 'hafta_1', 'hafta_2', ...  → o ayın N. haftası (ay-içi sıralı,
              ay sınırı geçmez, _week_of_month formülü ile)
          - 'default'      → diğer hepsi

    day_label: (eski parametre — kullanılmaz; backward-compat için tutuldu)
    ...
    Örnekler:
        ...
        _resolve_kadro(d, date_str='2026-02-02')  → 410  (Şubat'ın 2. haftası)
        _resolve_kadro(d, date_str='2026-02-09')  → ?    (Şubat hafta_3, dict yok → 380)
        _resolve_kadro(d, date_str='2026-02-23')  → 380  (default)

        # Karışık (tarih en üstte):
        d = {'2026-02-09': 500, 'hafta_3': 410, 'default': 380}
        _resolve_kadro(d, date_str='2026-02-09')  → 500  (tarih)
        _resolve_kadro(d, date_str='2026-02-10')  → 410  (hafta_3)
        _resolve_kadro(d, date_str='2026-02-16')  → 380  (hafta_4 yok → default)
    """
