# =============================================================================
# HAFTALIK MIP PİPELİNE - ACTUAL V8 (WEEKLY, HAFTAİÇİ)
# =============================================================================
#
# V8 = V7 + KADEMELİ FALLBACK LADDER (run_week_all_queues)
#
# V7 ile aynı MIP motoru. Tek değişiklik: infeasibility kurtarma stratejisi.
#
#   AŞAMA 1) Orijinal config — her şey default
#   AŞAMA 2) PER-DAY shrinkage azaltma (sadece bottleneck gün)
#            - Günler Erlang peak'ine göre sıralanır (büyükten küçüğe)
#            - En kritik gün için decrement +step uygulanır
#            - Hâlâ infeasible → o gün daha da azalt (floor'a kadar, default 0)
#            - Floor'a indi hâlâ infeasible → 2. kritik güne geç
#            - V7'deki UNIFORM (tüm günler) yerine GÜN-BAZLI: diğer günlerin
#              Erlang ihtiyacı bozulmadan sadece sorunlu gün gevşer.
#   AŞAMA 3) min_per_shift azaltma (uniform, tüm shift'lere)
#            - Tüm günler shrinkage=floor'da, hâlâ infeasible
#            - min_per_shift = default → default-step → ... → floor
#
# YENİ CONFIG ANAHTARLARI (queue_configs[Q]['mip']):
#   'weekly_shrinkage_fallback': {
#       'enabled': True, 'step': 0.10, 'floor': 0.0,
#       'per_day': True,      # V8'de YENİ — false = V7 uniform davranış
#   },
#   'weekly_min_per_shift_fallback': {       # V8'de YENİ
#       'enabled': True, 'step': 1, 'floor': 1,
#   },
#
# Geriye uyumlu: per_day=False ise V7 ile aynı uniform davranış.
# Pipeline'ın diğer tüm fonksiyonları (optimize_week, print_queue_report,
# distribute_surplus, balance_penalty vs.) V7 ile birebir aynı.
# =============================================================================
...
def run_week_all_queues(df_calls, df_shifts_by_queue, target_dates, config,
    ...
    for queue in queues:
        ...
        def _trial(stage_label, min_param):
            ...
            if verbose:
                print(f"\n  [trial #{idx}] {stage_label}  "
                      f"→ Erlang toplam={erl_total}  {min_summary}")

            stable_try, day_try, info_try = optimize_week(
                erl_pd, df_shifts, queue, target_dates, config,
                min_per_shift_override=min_arg, verbose=verbose,
            )
