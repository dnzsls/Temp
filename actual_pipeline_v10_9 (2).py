...
#   - Kuyruk bazlı olması gereken bloklar 'queue_configs' altına taşındı:
#       erlang, mip, rr_penalty, small_shift_penalty, slot_cap, hourly_report
#   - Bu blokların GLOBAL versiyonları silindi. Her kuyruk kendi değerlerine
#     sahiptir (kitle / kurumsal / gold).
#   - Pipeline bu değerleri doğrudan config['queue_configs'][queue][...] olarak
#     okur (eskiden deep-merge ile CONFIG üstüne yazılıyordu).
#   - V2 default'ları:
...
CONFIG = {
    ...
    # ---- PART-TIME ----
    # V2: PT devre dışı. Eğer aktif etmek istersen enabled: True yap ve
    # count'ları ayarla.
    'part_time': {
        'enabled': False,
    ...
    # =========================================================================
    # ---- QUEUE CONFIGS (KUYRUK BAZLI AYARLAR) ----
    # =========================================================================
    # Her kuyruk için kendi erlang / mip / rr_penalty / small_shift_penalty /
    # slot_cap / hourly_report bloklarını tanımlar.
    # Pipeline bu değerleri doğrudan config['queue_configs'][queue][...] olarak
    ...
    'queue_configs': {
        ...
        'kitle': {
            ...
            # Ardışık saatlerdeki shift giriş sayılarının birbirine yakın olmasını
            # teşvik eder. Penalty = |start[t] - start[t-1]| × penalty_per_diff
            # Inhouse ve outsource için AYRI AYRI uygulanır.
            'start_smoothing': {
                'enabled': True,
                'hours': {'start': '07:00', 'end': '20:00'},
                'companies': ['inhouse', 'outsource'],
                'penalty_per_diff': 0.5,
            },
            'hourly_report': {
                'rapor_etkisi': {
                    0: 0.03, 1: 0.03, 2: 0.03, 3: 0.03, 4: 0.03, 5: 0.03, 6: 0.03,
    ...
    'surplus_distribution': {
        'enabled': True,
        # PT hariç inhouse kadro (outsource için tavan yok — MIP zaten
        # outsource'u minimum kullanır, gereksiz kısıt infeasibility yaratır)
        'total_kadro': {
            'kitle':    {'inhouse': 460},
            'kurumsal': {'inhouse': 80},
            'gold':     {'inhouse': 50},
