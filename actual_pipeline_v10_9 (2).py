...
#          - Outsource maliyeti yüksek (queue_configs[<queue>]['mip']['cost_outsource'] = 10.0)
...
#
# Config: config_v2_weekday.py kullanın. V2 default'ları config dosyasında
# zaten uygulanmıştır:
#     - CONFIG['queue_configs'][<queue>]['mip']['cost_outsource'] = 10.0
...
def optimize_queue(erlang_by_slot, df_shifts, queue,
    ...
    # ---- SHIFT BAŞLANGIÇ YUMUŞATMA (START SMOOTHING) ----
    # Ardışık saatlerdeki shift girişlerinin sıçramalarını cezalandırır.
    # Her saatte (07:00, 07:30, 08:00, ...) "o saatte başlayan shift'lerdeki
    # toplam kişi sayısı" değişkeni oluşturulur, ardışık saatler arasındaki
    # |fark| LP'ye excess değişkeniyle lineer eklenir.
    ...
    if sm_cfg.get('enabled', False):
        ...
        # Her company için ayrı smoothing
        for comp in sm_companies:
            ...
            # Shift'leri başlangıç saatine göre grupla (aralık içinde olanlar)
            starts_by_hour = {}
            for s in comp_shifts:
                start_str = shift_cov[s]['start']  # 'HH:MM'
            ...
            # Ardışık saatler arasında |fark| penalty (LP'de: diff_pos + diff_neg)
        ...
        # cost listesine yeni terimleri eklediğimiz için amaç fonksiyonunu
        # yeniden yazmamız lazım (önceden prob += lpSum(cost) çağrılmıştı)
...
def run_queue_pipeline(df_calls, df_actual, df_shifts, target_date, queue,
    ...
    Kuyruk bazlı ayarlar config['queue_configs'][queue] altında olmalı:
      erlang, mip, rr_penalty, small_shift_penalty, slot_cap, hourly_report
    Bkz: config_v2_weekday.py
    ...
    # V2: queue_configs doğrudan okunuyor (deep-merge yok)
    # Her kuyruk kendi erlang/mip/rr_penalty/... bloklarına
    # config['queue_configs'][queue][...] ile erişir.
    if queue not in config.get('queue_configs', {}):
        raise ValueError(
            f"'{queue}' için queue_configs tanımı bulunamadı. "
            f"CONFIG['queue_configs']['{queue}'] altında erlang, mip, rr_penalty, "
            f"small_shift_penalty, slot_cap, hourly_report tanımlı olmalı."
        )
    ...
    if verbose:
        ...
        qc = config['queue_configs'][queue]
        cost_out = qc.get('mip', {}).get('cost_outsource', 1.0)
        or_cfg = config.get('outsource_ratio', {})
        or_q = or_cfg.get(queue) if isinstance(or_cfg, dict) else None
        if cost_out <= 1.0:
            print(f"   ⚠ V2 uyarısı: queue_configs[{queue}]['mip']['cost_outsource']={cost_out} düşük. "
                  f"V2 için 10.0 önerilir.")
        if or_q is not None:
...
def run_all_queues(df_calls, df_actual, df_shifts, target_date, config=CONFIG):
    ...
    for q, r in results.items():
        ...
        # Bu kuyruğun hourly_report'u → queue_configs
        q_hr = config['queue_configs'][q].get('hourly_report', {})
