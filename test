...
def optimize_queue(erlang_by_slot, df_shifts, queue,
    ...
    # Kadro tavanı (V9: per-day dict destekli — daily için target_date'ten resolve)
    kadro_cfg = config.get('surplus_distribution', {}).get('total_kadro', {}).get(queue, {})
    if target_date is not None:
        _date_s = pd.to_datetime(target_date).strftime('%Y-%m-%d')
        _day_l = _date_to_day_label(_date_s)
    else:
        _date_s = _day_l = None
    kadro_in = _resolve_kadro(kadro_cfg.get('inhouse', 0), _date_s, _day_l)
    kadro_out = _resolve_kadro(kadro_cfg.get('outsource', 0), _date_s, _day_l)
...
def print_queue_report(date, queue, erlang_by_slot, mip_info, actual,
    ...
    if has_rec or total_sf > 0:
        ...
        if has_rec:
            kadro_cfg = config.get('surplus_distribution', {}).get('total_kadro', {}).get(queue, {})
            # V9: per-day kadro destekli — o gün için resolve et (dict olabilir)
            date_s = mip_info.get('target_date')
            day_label = mip_info.get('day_label')
            in_cap = _resolve_kadro(kadro_cfg.get('inhouse', 0), date_s, day_label)
            out_cap = _resolve_kadro(kadro_cfg.get('outsource', 0), date_s, day_label)
...
def _resolve_kadro(kadro_value, date_str=None, day_label=None):
    """V9 — Kadro tavanını 3 katmanlı çözer: tarih > gün-of-week > default.

    Args:
        kadro_value: int (tüm günlerde aynı) VEYA dict.
            Dict ise key'ler: 'YYYY-MM-DD' (en spesifik), 'Mon'..'Sun', 'default'.
        date_str: '2026-02-09' formatında tarih
        day_label: 'Mon'..'Sun'

    Örnekler:
        _resolve_kadro(380, ...)  → 380
        _resolve_kadro({'default': 380, 'Mon': 400}, day_label='Mon')  → 400
        _resolve_kadro({'2026-02-02': 470, 'Mon': 460, 'default': 450},
                        date_str='2026-02-02', day_label='Mon')  → 470
        _resolve_kadro({'2026-02-02': 470, 'Mon': 460, 'default': 450},
                        date_str='2026-02-09', day_label='Mon')  → 460
    """
...
def optimize_week(erlang_by_slot_per_day, df_shifts, queue, target_dates, config,
    ...
    # Kadro tavanları (V9: per-day destekli):
    #   - INHOUSE: stable + her günün day-specific inhouse'u toplamı, o güne
    #     ait kadro_in_d'yi aşmasın. Stable her gün aynı kişiler olduğu için
    #     en kısıtlı günü çözer; diğer günlerin fark kadarı day-specific
    #     inhouse ile doldurulabilir.
    #   - OUTSOURCE: her gün ayrı kapasitede çalışabildiği için her gün
    #     ayrı per-day cap.
    # _resolve_kadro: scalar int OR dict {date|day|default → int}
    kadro_cfg = config.get('surplus_distribution', {}).get('total_kadro', {}).get(queue, {})
    kadro_in_raw = kadro_cfg.get('inhouse', 0)
    kadro_out_raw = kadro_cfg.get('outsource', 0)

    # Her gün için resolved kadro hesapla (verbose log için de kullanırız)
    kadro_in_per_day = {}
    kadro_out_per_day = {}
    ...
    if verbose and (any(v > 0 for v in kadro_in_per_day.values()) or
        ...
        # Sadece per-day değişen varsa logla (yoksa gürültü olmasın)
...
def _distribute_surplus_per_day(stable_assignments, day_specific_assignments,
    ...
    kadro_cfg = sd_cfg.get('total_kadro', {}).get(queue, {})
    # V9: per-day kadro destekli (int VEYA dict)
    total_in_kadro_raw = kadro_cfg.get('inhouse', 0)
    total_out_kadro_raw = kadro_cfg.get('outsource', 0)

    # En azından bir gün için bir kadro varsa devam et
    ...
    for d_label, info in info_per_day.items():
        ...
        # O günün target_date'ini al — per-day kadro çözümlemesi için lazım
